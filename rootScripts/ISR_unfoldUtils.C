...
void ISRUnfold::setNomFSRTUnfoldDensity(TString var, TString filepath, TString phase_name, TString fsr_correction_name)
{
        ...
        if( var == "Pt" )
        {
                ...
                // for closure test
                nomPtFSRUnfold_closure = new TUnfoldDensityV17(hmcGenGen,
                                               TUnfold::kHistMapOutputHoriz,
                                               regMode_FSR,
                                               TUnfold::kEConstraintArea,
                                               TUnfoldDensityV17::kDensityModeBinWidth,
        ...
        else if( var == "Mass" )
        {
                ...
                // for closure test
                nomMassFSRUnfold_closure = new TUnfoldDensityV17(hmcGenGen,
                                               TUnfold::kHistMapOutputHoriz,
                                               regMode_FSR,
                                               TUnfold::kEConstraintArea,
                                               TUnfoldDensityV17::kDensityModeBinWidth,
...
void ISRUnfold::doClosureTest(int detOrFSR_unfold, TString filepath, TString dir_name)
{
    ...
    {
        hpt_DYMC = (TH1*)filein->Get(dir_name + "/hist_ptll/histo_DYJetsnominal");
        hpt_DYMC->Add((TH1*)filein->Get(dir_name + "/hist_ptll/histo_DYJets10to50nominal"));
        hpt_DYMC->Add((TH1*)filein->Get(dir_name + "_DY_Fake/hist_ptll/histo_DYJetsnominal"), -1);
        hpt_DYMC->Add((TH1*)filein->Get(dir_name + "_DY_Fake/hist_ptll/histo_DYJets10to50nominal"), -1);
        nomPtUnfold_closure->SetInput(hpt_DYMC,   nominal_bias);

        hmass_DYMC = (TH1*)filein->Get(dir_name + "/hist_mll/histo_DYJetsnominal");
        hmass_DYMC->Add((TH1*)filein->Get(dir_name + "/hist_mll/histo_DYJets10to50nominal"));
        hmass_DYMC->Add((TH1*)filein->Get(dir_name + "_DY_Fake/hist_mll/histo_DYJetsnominal"), -1);
        hmass_DYMC->Add((TH1*)filein->Get(dir_name + "_DY_Fake/hist_mll/histo_DYJets10to50nominal"), -1);
...
TCanvas* ISRUnfold::drawISRresult(TString outpdf, bool doFit, TString genISRFile)
{
    ...
    if(genISRFile != "")
    {
        filein_dy = new TFile(genISRFile + "ISR_Gen_ISR_woNuisances.root");
        filein_dy_MG = new TFile(genISRFile + "ISR_Gen_ISR_woNuisances_MG.root");
        filein_dy_Powheg = new TFile(genISRFile + "ISR_Gen_ISR_woNuisances_Powheg.root");
        int lowMassEdge = 35;
        int highMassEdge = 45;
    ...
    grUnfolded_pre_fsr->SetLineStyle(1);
    grUnfolded_pre_fsr->GetYaxis()->SetRangeUser(5.,40.);
    grUnfolded_pre_fsr->GetXaxis()->SetLimits(10.,700.);
    grUnfolded_pre_fsr->GetXaxis()->SetMoreLogLabels(true);
    grUnfolded_pre_fsr->GetYaxis()->SetTitle("Average p_{T} (GeV)");
    grUnfolded_pre_fsr->GetYaxis()->SetTitleSize(25);
    ...
    grMC_preFSR->SetMarkerColor(kGray);
    grMC_preFSR->SetMarkerStyle(marker_ + 4);
    grMC_preFSR->SetMarkerSize(0.8);
    grMC_preFSR->SetLineStyle(1);
    grMC_preFSR->SetLineColor(kGray);
    ...
    grMC_preFSR_MG->SetMarkerColor(kRed);
    grMC_preFSR_MG->SetMarkerStyle(marker_+4);
    grMC_preFSR_MG->SetMarkerSize(0.8);
    grMC_preFSR_MG->SetLineStyle(1);
    grMC_preFSR_MG->SetLineColor(kRed);
    grMC_preFSR_MG->Draw("p same");

    TGraphErrors *grMC_preFSR_Powheg = new TGraphErrors(points-2, &mean_mass_powheg[2], &mean_pt_powheg[2], &mean_mass_stat_error_powheg[2], &mean_pt_stat_error_powheg[2]);
    grMC_preFSR_Powheg->SetMarkerColor(kBlue);
    grMC_preFSR_Powheg->SetMarkerStyle(marker_+2);
    grMC_preFSR_Powheg->SetMarkerSize(0.8);
    ...
    TLegend* leg_ = new TLegend(0.53, 0.7, 0.9, 0.9,"","brNDC");
    leg_->SetTextSize(0.035);
    leg_->SetFillStyle(0); // transparent
    leg_->SetBorderSize(0);
    leg_->AddEntry(grUnfolded_pre_fsr, "Unfolded data ", "pe");
    leg_->AddEntry(grMC_preFSR, "MC (aMC@NLO)", "pe");
    leg_->AddEntry(grMC_preFSR_MG, "MC (Madgraph)", "pe");
    leg_->AddEntry(grMC_preFSR_Powheg, "MC (Powheg)", "pe");
    leg_->Draw();
    ...
    if(doFit)
    {
        ...
        f1 = new TF1("f1", "[0]+[1]*log(x)", 20., 400.);
...
void ISRUnfold::drawSysSummaryPlots(TString outpdf, bool detector_unfold)
{
    ...
    c1 = new TCanvas("c_SysSummary_"+channel_name+"_"+year_string, "c_"+channel_name+"_"+year_string, 50, 50, 900, 700);
    c1->cd();
    gStyle->SetOptFit(0);
    ...
    while(it_for_sysName != end_for_sysName)
    {
        ...
        pv_graphs.at(nth_systematicValues)->SetLineStyle(1);
        pv_graphs.at(nth_systematicValues)->Draw(draw_option);
        pv_graphs.at(nth_systematicValues)->GetYaxis()->SetRangeUser(0.,2.);
        //pv_graphs.at(nth_systematicValues)->GetXaxis()->SetLimits(30.,500.);
        //pv_graphs.at(nth_systematicValues)->GetXaxis()->SetMoreLogLabels(true);
        pv_graphs.at(nth_systematicValues)->GetYaxis()->SetTitle("Absolute systematic uncertainty GeV");
        pv_graphs.at(nth_systematicValues)->GetXaxis()->SetTitle("Mass bin");
        //pv_graphs.at(nth_systematicValues)->SetName("postFSRUnfoldedData_"+channel_name+"_"+year_string);
        leg->AddEntry(pv_graphs.at(nth_systematicValues), it_for_sysName->first, "pl");
...
void ISRUnfold::drawClosurePlots(int detOrFSR_unfold, TString filepath, TString outpdf, TString var, int nthMassBin)
{
    ...
    if(var == "Pt" )
    {
        ...
        if(detOrFSR_unfold == 0)
        {
            ...
            dir_path = channel_name + "_fiducial_phase_post_FSR_dRp1_m"+ low_mass_edge[nthMassBin] + "to" + high_mass_edge[nthMassBin] + "/ptll/";
        }
        else if(detOrFSR_unfold == 1)
        {
            temp_tunfoldDensity = nomPtFSRUnfold_closure;
            dir_path = channel_name + "_full_phase_m"+ low_mass_edge[nthMassBin] + "to" + high_mass_edge[nthMassBin] + "/ptll_preFSR/";
        }
        else
        {
            exit(EXIT_FAILURE);
        ...
        hunfolded_data  = temp_tunfoldDensity->GetOutput("hunfolded_pt_closure",0,0,"pt[UO];mass[UOC"+ibinMass+"]",kTRUE);
        // hpreFSR_mc   =    sysPtUnfold["Closure"].at(0)->GetBias("histMC_pt_temp",0,0,"pt[UO];mass[UOC"+ibinMass+"]",kTRUE);
        hpreFSR_mc = (TH1*)filein->Get(dir_path + "histo_DYJets"); // get DY
    ...
    if(var == "Mass" )
    {
        ...
        if(detOrFSR_unfold == 0)
        {
            ...
            dir_path = channel_name + "_fiducial_phase_post_FSR_dRp1_m"+ low_mass_edge[nthMassBin] + "to" + high_mass_edge[nthMassBin] + "/mll_" + channel_name + "/";
        }
        else if(detOrFSR_unfold == 1)
        {
            temp_tunfoldDensity = nomMassFSRUnfold_closure;
            dir_path = channel_name + "_full_phase_m"+ low_mass_edge[nthMassBin] + "to" + high_mass_edge[nthMassBin] + "/mll_preFSR_" + channel_name + "/";
        }
        else
        {
            exit(EXIT_FAILURE);
...
void ISRUnfold::drawUnfoldedHists(TString outpdf, TString var, int nthMassBin, TString sysName, bool systematic, bool isFSRUnfold, bool fullSys)
{
    gROOT->SetBatch();
    ...
    // since the detector level histograms cannot be obtained from the probability matrix
    // they are read from the input histogram root file.
    TFile* filein = new TFile(hist_file_path);

    TH1* hDYReco_pt = NULL; // get DY
    TH1* hDYReco_mass = NULL;
    ...
    hDYReco_pt = (TH1*)filein->Get("detector_level/hist_ptll/histo_DYJetsTo" + PostFixForHist); // get DY
    hDYReco_pt->Add((TH1*)filein->Get("detector_level/hist_ptll/histo_DYJets10to50To" + PostFixForHist));
    if(do_normalization) hDYReco_pt->Scale(0.959939);
    hDYReco_pt-> SetDirectory(0);

    hDYReco_mass = (TH1*)filein->Get("detector_level/hist_mll/histo_DYJetsTo" + PostFixForHist);
    hDYReco_mass->Add((TH1*)filein->Get("detector_level/hist_mll/histo_DYJets10to50To" + PostFixForHist));
    if(do_normalization) hDYReco_mass->Scale(0.959939);
    hDYReco_mass-> SetDirectory(0);
    ...
    if(isFSRUnfold && systematic )
    {
        ...
        bool isFoldedSys = true;

        if(sysName != "QED_FSR")
        {
            makeSystBand(var, nthMassBin, sysName, fullSys, data_over_mc,
                      hfolded_data, hfolded_mc, hfolded_ratio,
                      hfolded_sys_err, hfolded_mc_sys_err, hfolded_ratio_sys_err, hfolded_ratio_sys_err_mc, isFoldedSys);
        }
...
void ISRUnfold::makeSystBand(const TString var, const int nthMassBin, const TString sysName, const bool fullSys, const bool data_over_mc,
                  ...
                  TH1* hunfolded_sys_err, TH1* hunfolded_mc_sys_err, TH1* hunfolded_ratio_sys_err, TH1* hunfolded_ratio_sys_err_mc, bool isFoldedSys)
{
    const TUnfoldBinningV17* temp_binning_gen_pt = nomPtUnfold->GetOutputBinning("Gen_Pt");
    const TUnfoldBinningV17* temp_binning_rec_pt = nomPtUnfold->GetInputBinning("Rec_Pt");
    const TUnfoldBinningV17* temp_binning_gen_mass = nomMassUnfold->GetOutputBinning("Gen_Mass");
    ...
    while(it != it_end)
    {
        ...
        if(var == "Pt")
        {
            ...
            if(it->first != "QED_FSR")
            {
                ...
                if(it->first == "Alt" && ibinMass == "0")
                {
                    hdatasys0_temp->GetXaxis()->SetRangeUser(28., 100.);
                    hdatasys0_temp->Smooth(1, "R");
                    hdatasys0_temp->GetXaxis()->SetRangeUser(0., 100.);
                }
