...
ISR2016_electron = {
    ...
    'DY'       : tnpSample('DYtoEE' ,       myinputDir + 'DYJets_cat_v8-0-7_1.root', isMC = True, isSig = True, isInc = True),
    'DY10to50' : tnpSample('DYtoEE10to50' , myinputDir + 'ISRee_unfolding_DYJets_10to50_cat_v8-0-7.root', isMC = True, isSig = True, isInc = True),
    'BKG'      : tnpSample('BKG' ,          myinputDir + 'ISRee_unfolding_background.root', isMC = True, isSig = False),
}

# for 50 DY ntuples
for i in range(2,51):
	ISR2016_electron['DY%s' % str(i)] = tnpSample('DYtoEE_%s' % str(i) , myinputDir + 'DYJets_cat_v8-0-7_%s.root' % str(i), isMC = True, isSig = True, isInc = True)
